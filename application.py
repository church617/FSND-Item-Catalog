...
def gconnect():
    ...
    user_token = getUserToken(data["email"])
    if not user_token:
        user_token = createUser(login_session)
    login_session['user_id'] = user_token
...
def getUserInfo(user_token):
    user = session.query(User).filter_by(user_token=user_token).one()
    return user
